...
function createOpieHarness() {
  const harness = new THREE.Group();
  harness.name = "opie-walking-harness";
  ...
  const frontBand = new THREE.Mesh(
    new THREE.TorusGeometry(0.185, 0.022, 10, 36),
    coral,
  );
  frontBand.rotation.x = Math.PI / 2;
  frontBand.scale.z = 0.78;
  frontBand.position.y = 0.7;
  harness.add(frontBand);

  const bodyBand = new THREE.Mesh(
    new THREE.TorusGeometry(0.225, 0.026, 10, 36),
    teal,
  );
  bodyBand.rotation.x = Math.PI / 2;
  bodyBand.scale.z = 0.76;
  bodyBand.position.y = 0.48;
  harness.add(bodyBand);

  addCylinderBetween(
    harness,
    new THREE.Vector3(-0.17, 0.48, 0),
    new THREE.Vector3(-0.145, 0.7, 0),
    0.019,
    coral,
  );
  addCylinderBetween(
  ...
  const backPlate = new THREE.Mesh(
    new THREE.BoxGeometry(0.13, 0.035, 0.24),
    coral,
  );
  backPlate.position.set(0, 0.72, -0.01);
  backPlate.castShadow = true;
  harness.add(backPlate);

  const buckle = new THREE.Mesh(
    new THREE.TorusGeometry(0.047, 0.012, 8, 24),
    brass,
  );
  buckle.rotation.x = Math.PI / 2;
  buckle.position.set(0, 0.75, -0.02);
...
function createGameAudio(): AudioController {
  ...
  return {
    ...
    setCharacter: (nextCharacter) => {
      ...
      if (engine) engine.type = character === "emmy" ? "sawtooth" : "sine";
    },
    setEngine: (speed, isDriving) => {
      ...
      engineGain.gain.setTargetAtTime(
        enabled && isDriving && character === "emmy"
          ? 0.035 + Math.min(velocity / 220, 0.065)
          : 0.0001,
        context.currentTime,
        0.09,
      );
...
export function PennRunGame() {
  ...
  useEffect(() => {
    ...
    characterLoader.load(
      `${ASSET_BASE}models/little-car-color.glb?v=3`,
      (gltf) => {
    ...
    characterLoader.load(
      `${ASSET_BASE}models/opie-walking.glb?v=1`,
      (gltf) => {
        const model = gltf.scene;
        model.traverse((object) => {
          if (!(object instanceof THREE.Mesh)) return;
        ...
        opieMotion.add(model);
        opieMotion.add(createOpieHarness());
        setOpieReady(true);
      },
      undefined,
      () => {
    ...
    new ThreeMFLoader().load(
      `${ASSET_BASE}models/cap-hill/penelopes-tiny-denver-race.3mf?v=1`,
      (cityMap) => {
    ...
    const animate = (now: number) => {
      ...
      if (startedRef.current && !finishLocked) {
        ...
        if (throttle !== 0) speed += throttle * 4.2 * delta;
        else speed *= Math.pow(0.18, delta);
        speed = THREE.MathUtils.clamp(speed, -2.4, 5.5);
        ...
        const nextX = car.position.x + Math.sin(heading) * speed * delta;
        const nextZ = car.position.z - Math.cos(heading) * speed * delta;
      ...
      gameAudio.setEngine(speed, startedRef.current && !finishLocked);
  ...
  const chooseCharacter = (nextCharacter: CharacterId) => {
    ...
    setToast(
      nextCharacter === "opie"
        ? "Opie’s harness is clipped in — ready to walk the loop"
      ...
      {!started && (
        <section className="start-screen">
          <div className="start-card">
                ...
                >
                  ...
                  <span className="character-copy">
                    ...
                    <span className="character-detail">
                      {opieReady ? "WALKING HARNESS" : "GETTING READY…"}
                    </span>
                  </span>
                </button>
              </div>
            ...
            <small>
              Emmy drives · Opie walks · Keyboard + touch controls
            </small>
          </div>
        </section>
      )}
